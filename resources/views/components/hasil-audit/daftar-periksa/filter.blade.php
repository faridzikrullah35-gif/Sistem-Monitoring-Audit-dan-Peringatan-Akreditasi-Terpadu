<div class="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-3">
    ...
    <div class="flex items-end gap-2">
        ...
        <a href="{{ route('hasil-audit.daftar-periksa.print', request()->query()) }}"
           target="_blank"
           id="btn-print"
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function updatePrintUrl() {
        ...
        printBtn.href = `{{ route('hasil-audit.daftar-periksa.print') }}?${params}`;
    }

    function fetchData() {
        const params = new URLSearchParams(getFilters()).toString();
        fetch(`{{ route('hasil-audit.filter') }}?${params}`)
            .then(res => res.text())
            .then(html => {
