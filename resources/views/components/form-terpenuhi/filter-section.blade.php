...
<div class="mb-5 flex flex-wrap items-center gap-4 rounded-lg bg-gray-50 p-4 dark:bg-gray-900/50">
    ...
    {{-- SELECT - support light/dark --}}
    <select
        x-model="selectedYear"
        @change="filterByYear"
        class="rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm
               text-gray-700 transition-colors focus:border-blue-500 focus:outline-none
               focus:ring-2 focus:ring-blue-500/20 dark:border-gray-700
    ...
    {{-- RESET BUTTON - support light/dark --}}
    <button
        type="button"
        @click="resetFilter"
    ...
    {{-- CETAK BUTTON - support light/dark --}}
    <a
        x-show="showCetak"
        x-transition.opacity.duration.200ms
        :href="`{{ route('form-terpenuhi.print') }}?tahun_akademik_id=${selectedYear}`"
        target="_blank"
    ...
    {{-- INFO TEXT - support light/dark --}}
    <div class="text-xs text-gray-500 dark:text-gray-400">
        <span x-show="selectedYear !== 'semua'">Filter tahun akademik aktif</span>
        <span x-show="selectedYear === 'semua'">Menampilkan semua data</span>
    </div>
</div>

<script>
function filterTerpenuhiComponent() {
    return {
        selectedYear: 'semua',
        showCetak: false,
        ...
        async filterByYear() {
            ...
            try {
                let url = new URL('{{ route("form-terpenuhi") }}');
                ...
                document.getElementById('terpenuhiTableContainer').innerHTML =
                    result.table;
                ...
                console.error('Filter terpenuhi gagal:', error);
            }
        },
